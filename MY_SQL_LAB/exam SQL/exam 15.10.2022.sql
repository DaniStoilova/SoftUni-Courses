...
SELECT `id`, `name`, COUNT(op.`order_id`) AS 'count' FROM `products` AS p
JOIN `orders_products` AS op ON op.`product_id` = p.`id`
GROUP BY p.`id`
HAVING `count` >= 5
ORDER BY `count` DESC, `name`;

SELECT t.`id` AS `table_id`, t.`capacity`, COUNT(oc.`client_id`)
AS 'count_clients',
CASE
...
-- 11
CREATE PROCEDURE udp_happy_hour(tipe VARCHAR(50))
BEGIN
UPDATE `products` as p
SET `price` = `price` * 0.80 
WHERE `price` >=  10 AND p.`type` = tipe;
END
